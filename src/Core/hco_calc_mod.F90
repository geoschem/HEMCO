       ...
       IF ( HcoState%Options%NegFlag /= 2 ) THEN
          ...
          IF ( ANY(TmpFlx < 0.0_hp) ) THEN
             ...
             IF ( HcoState%Options%NegFlag == 1 ) THEN
                ...
                IF ( HcoState%Config%doVerbose ) IF ( HcoState%Config%doVerbose ) CALL HCO_WARNING( MSG, RC )
...
!
    ...
    IF ( UseLL /= 1 ) THEN
       ...
       IF ( HcoState%Config%doVerbose ) IF ( HcoState%Config%doVerbose ) CALL HCO_WARNING( MSG, RC, THISLOC=LOC )
    ENDIF
